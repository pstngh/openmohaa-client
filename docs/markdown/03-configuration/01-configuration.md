...
which also holds the configuration files when the installation directory isn't
writable. This behavior can be customized:
...
camera, or when gameplay suppresses the HUD. Mounted guns and vehicle turrets
keep it even though they use a zoomed camera, and aiming at a teammate shows
the stock friendly crosshair (`cg_crosshair_friend`) instead, as without the
overlay.
