...
class EntityFrameWorkTests(unittest.TestCase):
    ...
    def test_out_of_range_parent_is_ignored(self):
        snapshot = [{"number": 2, "parent": 65535, "type": 0}]

        added, multibeams = traverse_entities(snapshot)

        self.assertEqual(added, [2])
        self.assertEqual(multibeams, [])
    ...
    def test_entity_state_uses_generation_markers_instead_of_full_clear(self):
        ...
        self.assertIn("(unsigned int)parent < MAX_ENTITIES", add_packet_entities)
